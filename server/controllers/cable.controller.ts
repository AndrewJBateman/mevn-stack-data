...
// Find all cables
export const displayAllCablesHandler = async (
  req: Request,
  res: Response
): Promise<any> => {
  try {
    const cables = await Cable.find();
		// const result = 
		console.log('type of promise: ', typeof(cables), cables);
    if (!cables) {
			const errorRes = res.status(404).json({
        message: "Cables not found",
      });
			console.log('errorRes type & value: ', typeof(errorRes), errorRes);
			return errorRes;
    }
    res.json(cables);
  } catch (error) {
...
export const deleteCableHandler = async (
  req: Request,
  res: Response
